...
function AppleMusicIcon() { return <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M12.152 6.896c-.948 0-2.415-1.078-3.96-1.04-2.04.027-3.91 1.183-4.961 3.014-2.117 3.675-.546 9.103 1.519 12.09 1.013 1.454 2.208 3.09 3.792 3.039 1.52-.065 2.09-.987 3.935-.987 1.831 0 2.35.987 3.96.948 1.637-.026 2.676-1.48 3.676-2.948 1.156-1.688 1.636-3.325 1.662-3.415-.039-.013-3.182-1.221-3.22-4.857-.026-3.04 2.48-4.494 2.597-4.559-1.429-2.09-3.623-2.324-4.39-2.376-2-.156-3.675 1.09-4.61 1.09zM15.53 3.83c.843-1.012 1.4-2.427 1.245-3.83-1.207.052-2.662.805-3.532 1.818-.689.827-1.316 2.233-1.132 3.61 1.35.105 2.628-.624 3.419-1.598z"/></svg> }
  ...
  return (
    <div className={`modal-overlay ${isClosing ? 'closing' : ''}`} onClick={handleClose}>
      <div 
        ...
        style={{ 
          width: '100%', maxWidth: 900, height: '85vh', borderRadius: 32, position: 'relative',
          ...
          <div className="modal-right">
            ...
            {selectedTrack ? (
              <div className="animate-in" style={{ display: 'flex', flexDirection: 'column', gap: 32 }}>
                <button onClick={() => setSelectedTrack(null)} className="glass-btn glass-pill-sm" style={{ alignSelf: 'flex-start', gap: 10 }}>
                  <ArrowLeftIcon /> <span>Back to Tracklist</span>
                </button>
                <div>
                  <h3 style={{ fontFamily: 'var(--font-display)', fontSize: 32, fontWeight: 700, color: 'var(--text-main)', marginBottom: 32, lineHeight: 1.1 }}>
                    {selectedTrack.title}
                  </h3>
              ...
              <div className="animate-in">
                ...
                <div style={{ display: 'flex', gap: 12, marginBottom: 40, flexWrap: 'wrap' }}>
                  ...
                  {project?.appleMusicUrl && (
                    <a href={project.appleMusicUrl} target="_blank" rel="noreferrer" className="glass-btn glass-pill-sm" style={{ gap: 8, color: '#FA243C' }}>
                      <AppleMusicIcon /> <span>Listen on Apple Music</span>
                    </a>
                  )}
                  {project?.youtubeUrl && (
